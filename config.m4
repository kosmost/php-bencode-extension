...
if test "$PHP_CG_BCODE" != "no"; then
  ...
  dnl [
  dnl   PHP_ADD_LIBRARY_WITH_PATH($LIBNAME, $CG_BCODE_DIR/lib, CG_BCODE_SHARED_LIBADD)
  dnl   AC_DEFINE(HAVE_CG_BCODELIB,1,[ ])
  dnl ],[
  dnl   AC_MSG_ERROR([wrong cg_bcode lib version or lib not found])
  dnl ],[
  dnl   -L$CG_BCODE_DIR/lib -lm -ldl
  dnl ])
  dnl
  dnl PHP_SUBST(CG_BCODE_SHARED_LIBADD)

  PHP_NEW_EXTENSION(cg_bcode, cg_bcode.c, $ext_shared)
fi
